    ...
    (catch 'found
      (while (lua-find-regexp search-direction lua-indentation-modifier-regexp)
        ...
        (let ((found-token (match-string 0))
          ...
          ;; no - then there is a nested block. If we were looking for a
          ;; block begin token, found-token must be a block end token;
          ;; likewise, if we were looking for a block end token, found-token
          ;; must be a block begin token, otherwise there is a grammatical
          ;; error in the code.
          (if (not (and
...
at the current point. Returns the point position of the first
character of the matching token if successful, nil otherwise."
...
Returns the point, or nil if it reached the beginning of the
buffer"
  (catch 'found
    (beginning-of-line)
    (while t
...
(defun lua-last-token-continues-p ()
  ...
  (let (line-begin
    ...
    (save-excursion
      ...
      (end-of-line)
      (setq line-end (point))
      ;; we need to check whether the line ends in a comment and skip that
      ;; one.
...
More specifically, return the point in the line that is
continued.  The criteria for a continuing statement are:

* the last token of the previous line is a continuing op, OR the
  first token of the current line is a continuing op
"
  (let ((prev-line nil))
    (save-excursion
      (if parse-start (goto-char parse-start))
...
lua-calculate-indentation-info. Don't use standalone."
...
The effect of each token can be either a shift relative to the
current indentation level, or indentation to some absolute
column. This information is collected in a list of indentation
info pairs, which denote absolute and relative each, and the
shift/column to indent to."
...
(defun lua-accumulate-indentation-info (info)
  "Accumulates the indentation information previously calculated
by lua-calculate-indentation-info. Returns either the relative
indentation shift, or the absolute column to indent to."
...
of blocks subtract lua-indent-level once each. This function is
used to determine how the indentation of the following line
relates to this one."
  (if parse-start (goto-char parse-start))
  (let ((case-fold-search nil)
    ...
    (if (eq (car indentation-info) 'absolute)
        (- (cdr indentation-info)
           ...
           ;; statement or next line cont. this line This is for aesthetic
           ;; reasons: the indentation should be dosomething(d + e + f + g)
           ;; not dosomething(d + e + f + g)"
           (save-excursion
...
(defconst lua-left-shift-regexp
  ;; This is the code we used to generate the regexp: ("else", "elseif",
  ;; "until" followed by whitespace, or "end"/closing brackets followed by
...
Look for an uninterrupted sequence of block-closing tokens that
starts at the beginning of the line. For each of these tokens,
shift indentation to the left by the amount specified in
lua-indent-level."
  (let (line-begin
  ...
  (let ((pos (point))
    ...
    (save-excursion
      ...
      (if (bobp)          ; If we're at the beginning of the buffer, no change.
        ...
        ;; This code here searches backwards for a "block beginning/end" It
        ;; snarfs the indentation of that, plus whatever amount the line was
        ;; shifted left by, because of block end tokens. It then adds the
        ;; indentation modifier of that line to obtain the final level of
        ;; indentation.  Finally, if this line continues a statement from the
        ...
        (if (lua-backwards-to-block-begin-or-end)
            ...
            (max (+ (current-indentation)
                    ...
                    shift-amt)
                 0)
          ;; Failed to find a block begin/end.  Just use the previous line's
          ;; indent.
          (goto-char pos)
          (beginning-of-line)
          (forward-line -1)
...
forward to Nth following beginning of proc.  Returns t unless
...
With argument, do it that many times.  Negative argument -N means
move back to Nth preceding end of proc.
...
(defun lua-postprocess-output-buffer (buf start &optional lua-stdin-line-offset)
  "Highlight tracebacks found in buf. If an traceback occurred
...
(defun lua-calculate-state (arg prevstate)
  ;; Calculate the new state of PREVSTATE, t or nil, based on arg. If arg is
  ;; nil or zero, toggle the state. If arg is negative, turn the state off,
  ;; and if arg is positive, turn the state on
  (if (or (not arg)
...
Optional numeric ARG, if supplied, turns on electric indentation
when positive, turns it off when negative, and just toggles it
when zero or left out."
  (interactive "P")
  ...
  (save-match-data
   ...
   (while (> count 0)
     ...
     (if (looking-at (regexp-opt block-start 'words))
       ...
       ;; If the current keyword is not a "begin" keyword, then just perform
       ;; the normal forward-sexp.
       (forward-sexp 1))
     (setq count (1- count))))))
